...
public class OptionMatchTests
{
    ...
    {
        Assert.Throws<ArgumentNullException>(() => _ = some.Match<int>(null, () => -1));
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => _ = none.Match(x => x + 1, null));
    }
    ...
    {
        Assert.Throws<InvalidOperationException>(() => _ = some.Match(
            x => (string)null,
            () => "fallback"
        ));
    }
    ...
    {
        Assert.Throws<InvalidOperationException>(() => _ = none.Match(
            x => "ok",
            () => (string)null
        ));
    }
