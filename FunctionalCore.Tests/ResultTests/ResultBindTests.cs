...
{
    private Result<string, int> _resOk1;
    private Result<string, int> _resFail1;


   [SetUp]
    public void Setup()
    {
        _resOk1 = Result<string, int>.Ok(5);
        _resFail1 = Result<string, int>.Fail("error");
    }
    ...
    {
        var res = _resOk1.Bind(x => Result<string, int>.Ok(x + 1));
    ...
    {
        var res = _resOk1.Bind(x => Result<string, int>.Fail("bind error"));
    ...
    {
        var res = _resFail1.Bind(x => Result<string, int>.Ok(x + 1));
    ...
    {
        ;
        Assert.Throws<ArgumentNullException>(() => _ = _resOk1.Bind<string>(null));
    }
